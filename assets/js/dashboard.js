...
const DEMO_DATA={
  ...
  summary:{totalRegistrations:312,confirmedRegistrations:268,classCount:23,sponsorCashTotal:60000000,sponsorInKindTotal:26400000,sponsorTotal:86400000,topClass:"KTB",topClassCount:28,checkedInCount:0,notCheckedInCount:268,checkinRate:0,topCheckinClass:"",topCheckinClassCount:0},
...
function renderSummary(s={}){
  document.querySelectorAll("[data-kpi]").forEach(el=>{const k=el.dataset.kpi;if(k==="classCount")el.textContent=`${fmt(s[k]||0)}/40`;else if(k==="sponsorTotal")animate(el,s[k],money);else if(k==="topClass")el.textContent=s[k]||"--";else animate(el,s[k])});
  ...
  const cash=Number(s.sponsorCashTotal||0),inKind=Number(s.sponsorInKindTotal||0);
  const sponsorBreakdown=document.getElementById("sponsorBreakdown");
  if(sponsorBreakdown)sponsorBreakdown.textContent=`Tiền mặt ${money(cash)} · Hiện vật ${money(inKind)}`;
  document.getElementById("checkedInCount").textContent=fmt(s.checkedInCount||0);
  document.getElementById("notCheckedInCount").textContent=fmt(Math.max(0,s.notCheckedInCount||0));
  document.getElementById("checkinRate").textContent=`${Number(s.checkinRate||0)}%`;
  document.getElementById("topCheckinClass").textContent=s.topCheckinClass||"--";
  document.getElementById("topCheckinClassNote").textContent=s.topCheckinClassCount?`${fmt(s.topCheckinClassCount)} người đã check-in`:"Chưa có dữ liệu";
}
...
function renderFeed(id,items=[],sponsor=false){const c=document.getElementById(id),a=items.slice(0,DASHBOARD_CONFIG.RECENT_LIMIT);if(!a.length){c.innerHTML='<div class="dash-empty">Chưa có dữ liệu.</div>';return}c.innerHTML=a.map(x=>`<div class="dash-feed-item"><div class="dash-feed-item__avatar">${sponsor?"💛":esc(initials(x.name))}</div><div><div class="dash-feed-item__title">${esc(x.name||"Thành viên K15")}</div><div class="dash-feed-item__subtitle">${esc(sponsor?(x.subtitle||x.tier||"Đồng hành"):(x.className||"Chưa rõ lớp"))}</div></div><div class="dash-feed-item__value">${sponsor?money(x.totalValue||x.amount||0):esc(x.status||"Đã đăng ký")}<span class="dash-feed-item__time">${ago(x.createdAt)}</span></div></div>`).join("")}
